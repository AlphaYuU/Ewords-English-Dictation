...
ipcMain.handle("database:query", (_event, request: DatabaseQuery) => {
  const database = getDatabase();
  switch (request?.resource) {
    case "bootstrap":
      return { ok: true, data: readBootstrap(database) };
    case "libraries":
      return { ok: true, data: findLibraries(database) };
    case "library-detail":
      return {
        ok: true,
        data: {
    ...
    case "dictionary-search":
      return { ok: true, data: searchDictionary(database, request.query, request.limit ?? 24) };
    case "dictionary-entry":
      return { ok: true, data: findDictionaryEntryWithExamples(database, request) };
    case "search-history":
      return { ok: true, data: findSearchHistory(database, request.limit ?? 12) };
    case "record-search":
      recordSearch(database, request.query, request.entryId);
      return { ok: true, data: null };
    case "settings":
      return { ok: true, data: getSettings(database) };
    case "create-library":
    ...
    case "delete-history":
      ...
      return { ok: true, data: readBootstrap(database) };
    default:
      return { ok: false, error: "UNKNOWN_DATABASE_RESOURCE" };
  }
});
