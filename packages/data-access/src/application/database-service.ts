...
import { ensureDatabaseFile, initializeApplicationDatabase } from "./database-lifecycle";
import type { ApplicationDatabaseService, ApplicationDatabaseServiceOptions, DatabaseQuery, DatabaseResponse } from "./types";
...
export function createApplicationDatabaseService(options: ApplicationDatabaseServiceOptions): ApplicationDatabaseService {
  let db: DatabaseSync | null = null;
  const getDatabase = () => {
    if (db) return db;
    mkdirSync(path.dirname(options.dbPath), { recursive: true });
    ensureDatabaseFile(options.dbPath, options);
    db = new DatabaseSync(options.dbPath);
    initializeApplicationDatabase(db, options);
    return db;
  };
  return {
    handleQuery: (request) => {
